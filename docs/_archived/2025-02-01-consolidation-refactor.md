...
const schema = z
  ...
  .refine((data) => !(data.mode === 'dedicated' && !data.endpointId), {
    message: 'endpointId required for dedicated mode',
    path: ['endpointId'],
  });
...
describe('OCIProviderSettingsSchema', () => {
  ...
  it('accepts dedicated mode with endpointId', () => {
    const settings = {
      servingMode: 'dedicated',
      endpointId:
        'ocid1.generativeaiendpoint.oc1..aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
    };
    expect(OCIProviderSettingsSchema.safeParse(settings).success).toBe(true);
  });
});
...
\*_Step 2: Ensure it includes packages/_
Expected content:
...
## Phase 6: Deprecation of opencode-oci-genai
...
| ----- | -------------- | -------- | ------------------------------------------------ |
| 6.1   | Feb 1, 2025    | COMPLETE | Consolidation complete, all packages migrated    |
| 6.2   | Feb 1-7, 2025  | ACTIVE   | Verification period - run both repos in parallel |
| 6.3   | Feb 8-14, 2025 | PENDING  | Documentation updates with deprecation notices   |
| 6.4   | Feb 15, 2025   | PENDING  | Archive repository                               |
| 6.5   | Mar 1, 2025    | PENDING  | Final cleanup and deletion                       |

### Task 6.1: Consolidation Complete (DONE)

**Completed:** February 1, 2025

- [x] All packages copied to oci-genai-provider
- [x] Dependencies updated to workspace references
- [x] Build verification passed for all packages
- [x] Test suites running and passing
- [x] CHANGELOG updated
- [x] Dashboard created (`oci-genai-dashboard.html`)

### Task 6.2: Verification Period (ACTIVE)

**Duration:** February 1-7, 2025

**Verification Checklist:**

```bash
# In oci-genai-provider
cd ~/Projects/oci-genai-provider
pnpm build
pnpm test
pnpm typecheck
...
pnpm build --filter=oci-ai-chat
pnpm build --filter=cli-tool
```

**Manual Checks:**

- [ ] Run oci-ai-chat example locally
...
- [x] `opencode-oci-genai/README.md` - Add deprecation notice
- [ ] `oci-genai-provider/README.md` - Note consolidated packages
- [ ] Update any external documentation links

### Task 6.4: Archive Repository

**Steps:**

```bash
# Create final archive branch in source repo
cd ~/Projects/opencode-oci-genai
git checkout -b archive/final-state
git push origin archive/final-state

# Move to archived directory
mkdir -p ~/Projects/_archived
mv ~/Projects/opencode-oci-genai ~/Projects/_archived/opencode-oci-genai-20250201

# If GitHub repo exists, archive it via settings
# Settings > General > Danger Zone > Archive this repository
```

### Task 6.5: Final Cleanup

**After 30 days (March 1, 2025):**

```bash
# Delete archived directory
rm -rf ~/Projects/_archived/opencode-oci-genai-20250201

# Unpublish npm packages if any were published
# npm unpublish @acedergren/old-package-name --force
```

---

## Consolidated Repository Summary

**Primary Repository:** `oci-genai-provider`
**Location:** `~/Projects/oci-genai-provider`

### Packages (6)

| Package                           | Source       | Status               |
| --------------------------------- | ------------ | -------------------- |
| @acedergren/oci-genai-provider    | Original     | Core provider        |
| @acedergren/agent-state           | Consolidated | Session persistence  |
| @acedergren/opencode-oci-setup    | Consolidated | CLI wizard           |
| @acedergren/opencode-oci-genai    | Consolidated | OpenCode integration |
| @acedergren/oci-openai-compatible | Original     | OpenAI compatibility |
| @acedergren/test-utils            | Original     | Test utilities       |

### Examples (10)

| Example             | Description                               |
| ------------------- | ----------------------------------------- |
| oci-ai-chat         | Svelte 5 + Tailwind 4 chat (consolidated) |
| cli-tool            | Terminal REPL interface                   |
| chatbot-demo        | SvelteKit chatbot                         |
| nextjs-chatbot      | Next.js 14 integration                    |
| fraud-analyst-agent | AI agent with tools                       |
| rag-demo            | Semantic document retrieval               |
| rag-reranking-demo  | RAG with reranking                        |
| realtime-stt-demo   | Real-time speech-to-text                  |
| stt-demo            | Speech transcription                      |
| opencode-config     | OpenCode setup example                    |

### Dashboard

View the consolidated status at:

```
~/Projects/oci-genai-provider/oci-genai-dashboard.html
```
